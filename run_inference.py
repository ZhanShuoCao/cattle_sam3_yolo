...
用法示例:
    ...
    # 关闭 fragment merge（对比效果）
    python run_inference.py --image cow.jpg --config config.yaml --no-merge
"""
...
def parse_args():
    ...
    parser.add_argument("--conf", type=float, default=None,
                        help="检测置信度阈值")
    parser.add_argument("--owl-prompt", type=str, default=None,
                        help="OWL 文本提示（覆盖 config）")
    parser.add_argument("--no-detector-guided", action="store_true",
                        help="关闭 detector-guided 模式")
    parser.add_argument("--no-merge", action="store_true",
                        help="关闭 fragment merge")
...
def load_config(args) -> PipelineConfig:
    ...
    # 命令行覆盖
    if args.detector:
        config.detector_config.detector_type = args.detector
    ...
    if args.conf is not None:
        ...
        config.detector_config.owl_threshold = args.conf
    if args.owl_prompt:
        config.detector_config.owl_text_prompt = args.owl_prompt
    if args.no_detector_guided:
        config.detector_guided = False
    if args.no_merge:
        config.fragment_merge.enabled = False
    if args.debug:
...
def process_image(image_path: str, pipeline: CowSegmentationPipeline, args, config):
    ...
    print(f"  合并了 {result.num_merges} 个实例")
...
def main():
    ...
    print(f"  Detector:         {config.detector_config.detector_type}")
    print(f"  Detector-guided:  {config.detector_guided}")
    print(f"  Fragment merge:   {config.fragment_merge.enabled}")
