    ...
    def overlay_instances(
        ...
        *,
        ...
        for i in range(num_instances):
            ...
            # Always draw box with "id: X" label on the corner when boxes are present
            if boxes is not None:
                self.draw_box(boxes[i], alpha=1, edge_color=color)
                # Draw "id: X" label on box corner regardless of binary_masks
                box_label = f"id: {i}" if labels is None else labels[i]
                mark = self._draw_number_in_box(
                    boxes[i], box_label, color=color, label_mode=label_mode
                )
                marks.append(mark)

            if binary_masks is not None:
                # Still draw number inside mask for clarity
    ...
    def _draw_number_in_box(self, box, text, color, label_mode="1"):
        ...
        if label_mode == "a":
            try:
                ...
                pass  # text is already a string label like "id: 0"
        else:
            text = text
        self.draw_text(
            text,
    ...
    def _draw_number_in_mask(
        ...
        if label_mode == "a":
            try:
                ...
                pass  # text is already a string label
        else:
            text = text
